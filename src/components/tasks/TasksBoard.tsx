...
import { Plus, Search, Filter, Pencil, Trash2, GripVertical, List as ListIcon, LayoutDashboard, CheckSquare } from 'lucide-react';
...
const COLUMNS: { status: TaskStatus; color: string }[] = STATUS_ORDER.map(s => ({ status: s, color: STATUS_COLORS[s] }));
...
function DraggableCard({ task, clients, members, openEdit, setDeleteId, onComplete, onAdvance, isOverlay = false }: any) {
    ...
    const project = clients.find((c: any) => c.id === task.clientId);
    ...
    return (
        <div ref={setNodeRef} 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                 {project && (
                    <span style={{ fontSize: 10, fontWeight: 800, color: project.color || '#64748B', textTransform: 'uppercase', letterSpacing: '0.02em' }}>
                        {project.name}
                    </span>
                )}
...
export default function TasksBoard({ clientId }: { clientId?: string }) {
    ...
    const [form, setForm] = useState({
        title: '', description: '', clientId: clientId || clients[0]?.id || '',
        assigneeId: '',
    ...
    }, [tasks, searchQuery, filterClient, filterPriority, filterAssignee, filterStatus, filterClientStage, showOverdueOnly, hideCompleted, filterDate, clientId]);

    const openCreate = () => {
        setEditingTask(null);
        setForm({ title: '', description: '', clientId: clientId || clients[0]?.id || '', assigneeId: '', priority: 'Média', deadline: '', estimatedHours: 0, status: 'A Fazer', checklists: [] });
        setModalOpen(true);
    };
    ...
    const saveTask = () => {
        ...
        const project = clients.find(c => c.id === form.clientId);
    ...
    const handleComplete = (task: Task) => {
        ...
        toast(newStatus === 'Concluído' ? 'Task concluída! 🎉' : 'Task reaberta');
    };
    ...
    const handleAdvance = (task: Task) => {
        ...
        if (currentIdx < STATUS_ORDER.length - 1) {
            ...
            toast(`Task agora está em: ${nextStatus}`);
        }
    };
            ...
            {viewMode === 'kanban' ? (
                <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
                    ...
                    <DragOverlay>
                        {activeDragTask ? (
                            <DraggableCard 
                                task={activeDragTask} clients={clients} members={members} 
                                openEdit={openEdit} setDeleteId={setDeleteId} 
                                onComplete={handleComplete} 
                                onAdvance={handleAdvance} 
                                isOverlay 
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        <thead>
                            <tr style={{ background: '#F8FAFC', borderBottom: '1px solid #E2E8F0', fontSize: 12, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                ...
                                <th style={{ padding: '12px 16px', fontWeight: 600 }}>Projeto</th>
                        ...
                        <tbody>
                            {filteredTasks.length === 0 ? (
                                <tr><td colSpan={8} style={{ padding: 20, textAlign: 'center', color: '#94A3B8' }}>Nenhuma tarefa encontrada.</td></tr>
    ...
    const isOverdue = task.deadline && !['Concluído'].includes(task.status) && task.deadline < todayStr;
                                return (
                                    <tr key={task.id} 
                                        onClick={() => openEdit(task)}
                                        style={{ borderBottom: '1px solid #F1F5F9', transition: 'background 0.15s', cursor: 'pointer', background: isOverdue ? '#FFF1F2' : isToday ? '#FFFBEB' : 'white' }}>
                                         <td style={{ padding: '12px 16px' }}>
                                             <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                 {isOverdue && <span title="Atrasada" style={{ color: '#EF4444', fontSize: 12 }}>⚠️</span>}
                                                 {isToday && <span title="Vence hoje" style={{ color: '#F59E0B', fontSize: 12 }}>📅</span>}
                                                 <button
                                                     onClick={e => { e.stopPropagation(); handleComplete(task); }}
                                                     style={{
                                                         flexShrink: 0, width: 20, height: 20, borderRadius: 10, cursor: 'pointer',
                                                         border: task.status === 'Concluído' ? 'none' : '2px solid #CBD5E1',
                                                         background: task.status === 'Concluído' ? '#22C55E' : 'white',
                                                         display: 'flex', alignItems: 'center', justifyContent: 'center'
                                                     }}
                                                 >
                                                     {task.status === 'Concluído' && <span style={{ fontSize: 10, color: 'white', fontWeight: 900 }}>✓</span>}
                                                 </button>
                                                 <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                                     <span style={{ fontSize: 14, fontWeight: 600, color: task.status === 'Concluído' ? '#64748B' : '#0F172A', textDecoration: task.status === 'Concluído' ? 'line-through' : 'none' }}>{task.title}</span>
                                                     {task.checklists && task.checklists.length > 0 && (
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#94A3B8' }}>
                                                        <CheckSquare size={12} />
                                                        {task.checklists.reduce((acc, c) => acc + c.items.filter(i => i.completed).length, 0)}/
                                        ...
                                        <td style={{ padding: '12px 16px' }}>
                                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 8px', borderRadius: 6, background: '#F8FAFC', border: '1px solid #E2E8F0', fontSize: 12, fontWeight: 500, color: '#475569' }}>
                                                <div style={{ width: 6, height: 6, borderRadius: '50%', background: statusCol?.color || '#CBD5E1' }} />
                                                {task.status}
                                            </span>
                                        </td>
                                        <td style={{ padding: '12px 16px' }}>
            ...
            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingTask ? 'Editar Task' : 'Nova Task'}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div>
                            <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 6 }}>Projeto</label>
